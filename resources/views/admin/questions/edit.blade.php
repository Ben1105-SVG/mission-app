...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="text-xl font-semibold text-slate-800">Edit Question</h2>
            <p class="mt-1 text-sm text-slate-500">Update the question details below.</p>

            <form method="POST" action="{{ route('admin.questions.update', $question) }}" class="mt-6 space-y-5">
                @csrf
                @method('PATCH')
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input
                            ...
                            type="number"
                            min="1"
                            value="{{ old('group', $question->group) }}"
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="3"
                        class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-1 resize-none"
                    >{{ old('hint', $question->hint) }}</textarea>
                    @error('hint')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    >{{ old('options', $question->options ? (is_array($question->options) ? json_encode($question->options, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) : $question->options) : '') }}</textarea>
